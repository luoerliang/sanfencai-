...
TEN27_V4_PROFILE="27十期精确长码"
TEN27_V4_CORE_PROFILE="27V4核心23"
TEN27_V4_MOBILE_PROFILE="27V4机动4"
TEN27_V4_28_PROFILE="27V4第28码"
TEN27_V5_PROFILE="27十期严格冷肖杀码"
TEN27_V5_CORE_PROFILE="27V5核心23"
TEN27_V5_MOBILE_PROFILE="27V5后4"
TEN27_V5_28_PROFILE="27V5第28码"
F_DYNAMIC_V45_PROFILE="F动态v45"
F_DYNAMIC_V48_PROFILE="F动态v48真2of5"
TEN27_V6_PROFILE="27十期规则诊断版"
TEN27_V6_CORE_PROFILE="27V6核心23"
TEN27_V6_MOBILE_PROFILE="27V6后4"
TEN27_V6_28_PROFILE="27V6第28码"
TEN27_V6_RAW_PROFILE="27V6原始Top27"
F_DYNAMIC_V49_PROFILE="F动态v49独家救援"
TEN27_V7_PROFILE="27十期冷肖可行性"
TEN27_V7_CORE_PROFILE="27V7核心23"
TEN27_V7_MOBILE_PROFILE="27V7后4"
...
B27_V58_PROFILE="27B近20期尾数软评分v58"
C22_V58_PROFILE="22C独立优化v58"
F_DYNAMIC_V59_PROFILE="F动态v59误伤控制"
F_RAW_SAME_V59_PROFILE="F六模型同码数基准v59"
F_RAW23_V59_PROFILE="F六模型Top23参考v59"
F_CORE20_V60_PROFILE="F核心20贡献v60"
F_ADD21_V60_PROFILE="F第21码贡献v60"
F_ADD22_V60_PROFILE="F第22码贡献v60"
F_ADD23_V60_PROFILE="F第23码贡献v60"
F_DYNAMIC_V61_PROFILE="F动态v61四票绝对保留"
F_DYNAMIC_V62_PROFILE="F动态v62四票绝保强三票优先"
F_RAW_SAME_V62_PROFILE="F六模型同码数基准v62"
F_RAW23_V62_PROFILE="F六模型Top23参考v62"
F_CORE20_V62_PROFILE="F核心20贡献v62"
F_ADD21_V62_PROFILE="F第21码贡献v62"
F_ADD22_V62_PROFILE="F第22码贡献v62"
F_ADD23_V62_PROFILE="F第23码贡献v62"
A27_V62_PROFILE="A组27多策略池杀一头v62"
B27_V62_PROFILE="27B纯近10期尾数软评分v62"
...
async function copy27B(){
  const text=SPECIAL27B.join(',');
  try{ await navigator.clipboard.writeText(text); }
  catch(e){
    const ta=document.createElement('textarea');
    ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();
...
def init_db():
    with db_lock:
        ...
        for _sql in [
            ...
            "ALTER TABLE strategy_audit ADD COLUMN snapshot_json TEXT"
        ]:
            try:
                c.execute(_sql)
...
def _f_unique_model_value():
    """Measure recent 'only this model hit' value for T/Z/C/W/A.

    Used only for the two 1/5 rescue seats. Main model weights stay unchanged.
    """
    profiles=list(POOL_MODEL_PROFILES.values())
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        ...
        try:
            rows=c.execute(f"""SELECT target_issue,profile,hit24
                               ...
                               WHERE profile IN ({placeholders}) AND settled=1
                               ORDER BY CAST(target_issue AS INTEGER) DESC
                               LIMIT 500""",tuple(profiles)).fetchall()
        finally:
            c.close()
    ...
    for x in rows:
        ...
        if k:
            by_issue.setdefault(q,{})[k]=int(x["hit24"] or 0)

    ordered=sorted(by_issue,key=lambda q:int(q),reverse=True)
    perf=_pool_performance()
    pstats=perf.get("stats") or {}
    out={}
    for k in POOL_MODEL_PROFILES:
        u10=u30=0
        for idx,q in enumerate(ordered[:30]):
            hits=[m for m,v in by_issue[q].items() if v]
            if len(hits)==1 and hits[0]==k:
                u30+=1
                if idx<10:
                    u10+=1
        stable=float((pstats.get(k) or {}).get("composite",20/49))
        value=.62*(u10/10.0)+.38*(u30/30.0)+.18*stable
        out[k]={
          "unique10":u10,
          "unique30":u30,
          "value":round(value,5)
...
def _f_trio_value():
    """Real forward value of the 20 possible 3-model combinations.

    Used only to distinguish STRONG 3/6 from ordinary 3/6.
    A trio is valuable when its three specialist lists have jointly covered
    the real special number more often in locked pre-draw records.
    """
    ...
    with db_lock:
        ...
        try:
            rows=c.execute(f"""SELECT target_issue,profile,hit24
                               ...
                               WHERE profile IN ({placeholders}) AND settled=1
                               ORDER BY CAST(target_issue AS INTEGER) DESC
                               LIMIT 900""",tuple(profiles)).fetchall()
        finally:
            c.close()
    ...
    values=[]
    import itertools
    for trio in itertools.combinations(keys,3):
        usable=[q for q in issues if all(k in by[q] for k in trio)]
        all3=sum(1 for q in usable if all(by[q][k] for k in trio))
        n=len(usable)
        # Bayesian smoothing so tiny samples cannot become "strong" by accident.
        value=(all3+8*baseline)/(n+8) if n else baseline
        name="+".join(sorted(trio))
        out[name]={"n":n,"all3":all3,"value":round(value,5)}
        values.append(value)

    sv=sorted(values)
    median=sv[len(sv)//2] if sv else baseline
    q75=sv[min(len(sv)-1,int(len(sv)*.75))] if sv else baseline
    return out,median,q75,baseline
...
def _f_v62_compare_stats(limit=240):
    """Fair same-count comparison for the current v62 F algorithm."""
    profiles=(F_RAW_SAME_V62_PROFILE,F_DYNAMIC_V62_PROFILE,F_RAW23_V62_PROFILE)
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        c=connect()
        try:
    ...
    for q in issues:
        d=by[q]
        if F_RAW_SAME_V62_PROFILE in d and F_DYNAMIC_V62_PROFILE in d:
            n+=1
            rh=int(d[F_RAW_SAME_V62_PROFILE])
            fh=int(d[F_DYNAMIC_V62_PROFILE])
        ...
        if F_RAW23_V62_PROFILE in d:
            top23_n+=1
            top23_hits+=int(d[F_RAW23_V62_PROFILE])
...
def _f_v62_slot_stats():
    ...
    profiles=[
      F_CORE20_V62_PROFILE,F_ADD21_V62_PROFILE,
      F_ADD22_V62_PROFILE,F_ADD23_V62_PROFILE
    ]
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        c=connect()
    ...
    core=data[F_CORE20_V62_PROFILE]
    p21=data[F_ADD21_V62_PROFILE]
    p22=data[F_ADD22_V62_PROFILE]
    p23=data[F_ADD23_V62_PROFILE]
    return {
...
def _f_v62_swap_budget():
    ...
    4+/6 absolute preservation is OUTSIDE this budget.
    Strong 3/6, 2/6 and 1/6 share this controlled budget.
    """
    st=_f_v62_compare_stats(240)
    n=int(st.get("n",0))
    rescued=int(st.get("rescued",0))
    harmed=int(st.get("harmed",0))
    if n<20:
        return 1,"学习期：4+/6绝保外，最多1次普通调整",st
    if harmed>=rescued+2:
        return 0,"普通调整误伤偏多：仅保留4+/6绝保",st
    if rescued>=harmed+3:
...
def _predict20_hot(r, profile):
    ...
    The pure six-model ranking is the mother list.
    F is not allowed to freely reshuffle it anymore.

    Fair comparison:
      - raw_same_count = pure six-model TopN, where N == F code count
      - final F       = same N after at most 0/1/2 evidence-gated swaps
      - raw_top23     = coverage reference only, NOT used as the harm metric
    """
    nums=list(range(1,50))
    ...
    raw_ranked=sorted(nums,key=lambda n:(-pool_score.get(n,-1e9),n))
    raw_top23=raw_ranked[:23]

    # Consensus metadata remains useful, but is no longer allowed to dominate.
    protected4=[n for n in raw_ranked if support.get(n,0)>=4]
    soft3=[n for n in raw_ranked if support.get(n,0)==3]
    ...
    # --------------------------------------------------------
    # Dynamic count 19~23 based on the PURE ranking edge.
    # --------------------------------------------------------
    target_count=20
    count_reasons=["默认20码"]
    s20=float(pool_score.get(raw_ranked[19],0.0))
    edge_candidates=[]
    for idx in (20,21,22):
        n=raw_ranked[idx]
        gap=max(0.0,s20-float(pool_score.get(n,0.0)))
        qualifies=(gap<=.045 and support.get(n,0)>=3)
    ...
    gap19_20=max(
      0.0,
      float(pool_score.get(raw_ranked[18],0.0))
      -float(pool_score.get(raw_ranked[19],0.0))
    )
    if gap19_20>=.13 and support.get(raw_ranked[19],0)<=1:
        target_count=19
        count_reasons=["19/20纯模型分差明显，收缩19码"]
    elif target_count>20:
        count_reasons=[f"纯模型边缘接近，扩至{target_count}码"]

    target_count=max(19,min(23,target_count))
    baseline=list(raw_ranked[:target_count])
    selected=list(baseline)

    # --------------------------------------------------------
    # 4+/6 ABSOLUTE PRESERVATION
    # --------------------------------------------------------
    # Any number supported by at least 4 of the 6 formal models MUST be in F.
    # It is not subject to the normal swap budget, pair quality, edge gap,
    # or harm-controller gating.
    forced4_swaps=[]
    for in_code in protected4:
        if in_code in selected:
            continue

        # Prefer to displace the weakest NON-4+/6 code currently in F.
        drop_pool=[n for n in selected if support.get(n,0)<4]
        if drop_pool:
            out_code=min(
              drop_pool,
              key=lambda n:(
                support.get(n,0),
                float(weighted.get(n,0)),
                float(pool_score.get(n,0)),
                -raw_ranked.index(n)
              )
            )
            selected[selected.index(out_code)]=in_code
            forced4_swaps.append({
              "in":f"{in_code:02d}",
              "out":f"{out_code:02d}",
              "reason":"4+/6绝对保留"
            })
        else:
            # Extreme case: every current code is already 4+/6.
            # Absolute preservation has priority over the old 19~23 ceiling.
            selected.append(in_code)
            forced4_swaps.append({
              "in":f"{in_code:02d}",
              "out":"",
              "reason":"4+/6绝对保留·强共识溢出扩码"
            })

    # If absolute preservation expanded the list, keep that actual size.
    ...
    for n in raw_ranked[target_count:]:
        gap=max(
          0.0,
          float(pool_score.get(baseline[-1],0.0))
          -float(pool_score.get(n,0.0))
        )

        # 4+/6: strongest allowed intervention.
        if support.get(n,0)>=4 and gap<=.040:
            candidates.append((
              3,                           # evidence tier
              float(weighted.get(n,0)),
              float(pool_score.get(n,0)),
        ...
        # 3/6: split into STRONG and ordinary.
        # Ordinary 3/6 stays only as a soft score signal.
        # Strong 3/6 may use the controlled normal-adjustment budget.
        if support.get(n,0)==3 and gap<=.035:
            ...
            if len(supporters)==3:
                ...
                # Strong = upper-quartile historical trio OR clearly above median
                # plus strong current weighted support.
                strong=(tv>=trio_q75) or (
                  tv>=trio_median+.008 and float(weighted.get(n,0))>=.48
                )
                if strong:
                    candidates.append((
                      2.5,
    ...
    swaps=[]
    for tier,evidence,_p,in_code,reason in candidates:
        if len(swaps)>=swap_budget:
            break
        if in_code in selected:
            continue

        # Only the weakest few current codes are eligible to be displaced.
        # 4+/6 is absolute and can never be removed.
        ranked_selected=sorted(
          selected,
          key=lambda n:(float(pool_score.get(n,0)),support.get(n,0),float(weighted.get(n,0)),n)
        )
        drop_pool=[n for n in ranked_selected[:6] if support.get(n,0)<4]
        if not drop_pool:
            continue
        ...
        # Incoming must be genuinely close to outgoing on pure-model score.
        score_gap=float(pool_score.get(out_code,0))-float(pool_score.get(in_code,0))
        max_gap=.040 if tier>=3 else (.035 if tier>=2.5 else (.022 if tier>=2 else .015))
    ...
    # Preserve seat order for exact "core20 vs temporary +21/+22/+23" audit.
    selected_ordered=list(selected)
    selected=sorted(set(selected))
    raw_same_count=sorted(baseline)

    consensus_summary={
      "protected4_count":len(protected4),
      "protected4":[f"{n:02d}" for n in protected4],
      "protected4_actual_count":sum(1 for n in protected4 if n in selected),
      "soft3_count":len(soft3),
      "soft3":[f"{n:02d}" for n in soft3[:16]],
      "strong3_rule":"历史三模型组合质量 + 当前加权支持 + 边界分差",
      "trio_median":round(trio_median,5),
      "trio_q75":round(trio_q75,5),
      "protected2_count":0,
      "protected2_cap":0,
      "protected2":[],
      "protected2_actual_count":0,
      "protected2_actual":[],
      "protected2_pairs":{},
      "rescue1_count":0,
      "rescue1":[],
      "rescue1_actual_count":0,
      "rescue1_actual":[],
      "rescue1_models":[],
    ...
    return selected,{
      ...
      "core20_codes":selected_ordered[:20],
      "add21_code":selected_ordered[20] if len(selected_ordered)>=21 else None,
      "add22_code":selected_ordered[21] if len(selected_ordered)>=22 else None,
      "add23_code":selected_ordered[22] if len(selected_ordered)>=23 else None,
      "forced4_swaps":forced4_swaps,
      "forced4_count":len(forced4_swaps),
      "edge_rescue_swaps":swaps,
      ...
      "error_rescue_meta":{"ready":False,"reason":"只允许证据足够的受控替换"},
...
def _long27_v5_token(filt,round_start,segment=1,switches=0):
    waves=",".join(filt.get("killed_wave_parity") or [])
    head=str(filt.get("killed_head") or "")
    return (
      f"LONG27V5|waves={waves}|head={head}|round={round_start}"
      f"|segment={int(segment)}|switches={int(switches)}"
    )

def _long27_v5_parse(token):
    out={"waves":[],"head":"","round":"","segment":1,"switches":0}
    t=str(token or "")
    if not t.startswith("LONG27V5|"):
        return out
...
def _long27_v7_round_context(target_issue):
    with db_lock:
        c=connect()
        try:
            full=c.execute("""SELECT target_issue,special24,settled
                              ...
                              WHERE profile=?
                              ORDER BY CAST(target_issue AS INTEGER) ASC""",
                           (TEN27_V7_PROFILE,)).fetchall()
            cores=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V7_CORE_PROFILE,)).fetchall()
            tails=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V7_MOBILE_PROFILE,)).fetchall()
            audits=c.execute("""SELECT target_issue,regime,snapshot_json
                                FROM strategy_audit
                                WHERE profile=?
                                ORDER BY CAST(target_issue AS INTEGER) ASC""",
                             (TEN27_V7_PROFILE,)).fetchall()
    ...
    if not full:
        return {
          "new_round":True,"position":1,"round_start":target,
          "start_issue":target,"end_issue":_issue_add(target,9),
          "core23":[],"mobile4":[],"prev_filter":{},
          "prev_snapshot":{}
        }

    full_by_issue={str(x["target_issue"]):dict(x) for x in full}
    core_map={str(x["target_issue"]):_csv_nums(x["special24"]) for x in cores}
    tail_map={str(x["target_issue"]):_csv_nums(x["special24"]) for x in tails}
    audit_map={str(x["target_issue"]):dict(x) for x in audits}

    latest_issue=str(full[-1]["target_issue"])
    latest_audit=audit_map.get(latest_issue,{})
    latest_token=_long27_v5_parse(latest_audit.get("regime",""))
    latest_snap=_audit_snapshot_load(latest_audit.get("snapshot_json",""))
    round_start=str(latest_snap.get("round_start") or latest_token.get("round") or latest_issue)

    current_round=[]
    for x in full:
        q=str(x["target_issue"])
        a=audit_map.get(q,{})
        snap=_audit_snapshot_load(a.get("snapshot_json",""))
        tok=_long27_v5_parse(a.get("regime",""))
        rs=str(snap.get("round_start") or tok.get("round") or q)
        if rs==round_start:
            current_round.append(q)

    if target in full_by_issue:
        a=audit_map.get(target,{})
        snap=_audit_snapshot_load(a.get("snapshot_json",""))
        tok=_long27_v5_parse(a.get("regime",""))
        rs=str(snap.get("round_start") or tok.get("round") or target)
        members=[]
        for x in full:
            q=str(x["target_issue"])
            aa=audit_map.get(q,{})
            ss=_audit_snapshot_load(aa.get("snapshot_json",""))
            tt=_long27_v5_parse(aa.get("regime",""))
            if str(ss.get("round_start") or tt.get("round") or q)==rs:
                members.append(q)
        pos=(members.index(target)+1) if target in members else 1
        latest_for_round=members[-1] if members else target
        la=audit_map.get(latest_for_round,{})
        ls=_audit_snapshot_load(la.get("snapshot_json",""))
        lt=_long27_v5_parse(la.get("regime",""))
        return {
          "new_round":False,"position":pos,"round_start":rs,
          "start_issue":rs,"end_issue":_issue_add(rs,9),
          "core23":list(core_map.get(latest_for_round) or [])[:23],
          "mobile4":list(tail_map.get(latest_for_round) or [])[:4],
          "prev_filter":lt,"prev_snapshot":ls
        }

    if len(current_round)>=10:
        return {
          "new_round":True,"position":1,"round_start":target,
          "start_issue":target,"end_issue":_issue_add(target,9),
          "core23":[],"mobile4":[],
          "prev_filter":latest_token,"prev_snapshot":latest_snap
        }

    return {
      "new_round":False,"position":len(current_round)+1,"round_start":round_start,
      "start_issue":round_start,"end_issue":_issue_add(round_start,9),
      "core23":list(core_map.get(latest_issue) or [])[:23],
      "mobile4":list(tail_map.get(latest_issue) or [])[:4],
      "prev_filter":latest_token,"prev_snapshot":latest_snap
    }


def _long27_v6_round_context(target_issue):
    with db_lock:
        c=connect()
        try:
            full=c.execute("""SELECT target_issue,special24,settled
                              ...
                              WHERE profile=?
                              ORDER BY CAST(target_issue AS INTEGER) ASC""",
                           (TEN27_V6_PROFILE,)).fetchall()
            cores=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V6_CORE_PROFILE,)).fetchall()
            tails=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V6_MOBILE_PROFILE,)).fetchall()
            audits=c.execute("""SELECT target_issue,regime,snapshot_json
                                FROM strategy_audit
                                WHERE profile=?
                                ORDER BY CAST(target_issue AS INTEGER) ASC""",
...
def _long27_v5_round_context(target_issue):
    """A code change starts a NEW 10-period round and resets hit/miss display."""
    with db_lock:
        c=connect()
        try:
            full=c.execute("""SELECT target_issue,special24,settled
                              ...
                              WHERE profile=?
                              ORDER BY CAST(target_issue AS INTEGER) ASC""",
                           (TEN27_V5_PROFILE,)).fetchall()
            cores=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V5_CORE_PROFILE,)).fetchall()
            tails=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V5_MOBILE_PROFILE,)).fetchall()
    ...
    if not full:
        return {
          ...
          "start_issue":target,"end_issue":_issue_add(target,9),
          "core23":[],"mobile4":[],
          "prev_filter":{"waves":[],"head":"","round":"","segment":1,"switches":0}
        }
    ...
    latest_issue=str(full[-1]["target_issue"])
    latest_token=_long27_v5_parse(audit_map.get(latest_issue,""))
    round_start=str(latest_token.get("round") or latest_issue)

    current_round=[]
    for x in full:
        q=str(x["target_issue"])
        tok=_long27_v5_parse(audit_map.get(q,""))
        if str(tok.get("round") or q)==round_start:
            current_round.append(q)

    # If target already exists, return its round position.
    if target in full_by_issue:
        tok=_long27_v5_parse(audit_map.get(target,""))
        rs=str(tok.get("round") or target)
        members=[]
        for x in full:
            q=str(x["target_issue"])
            t=_long27_v5_parse(audit_map.get(q,""))
            if str(t.get("round") or q)==rs:
                members.append(q)
        pos=(members.index(target)+1) if target in members else 1
        latest_for_round=members[-1] if members else target
        return {
          ...
          "core23":list(core_map.get(latest_for_round) or [])[:23],
          "mobile4":list(tail_map.get(latest_for_round) or [])[:4],
          "prev_filter":_long27_v5_parse(audit_map.get(latest_for_round,""))
        }

    # 10 settled/predicted positions completed -> natural new round.
    if len(current_round)>=10:
        return {
          "new_round":True,"position":1,"round_start":target,
          "start_issue":target,"end_issue":_issue_add(target,9),
          "core23":[],"mobile4":[],
          "prev_filter":latest_token
        }
...
def _build_long27_v5_codes(r,profile,filt):
    """Strict clarified rule.

    Coldest 3 zodiacs:
      * exactly 2 codes EACH;
      * BOTH codes must belong to one of the TWO killed wave×parity groups;
      * non-killed wave×parity codes are NEVER allowed for a cold zodiac.

    Head rule:
      * first try to avoid the killed 0/4 head;
      * only if a cold zodiac cannot supply 2 killed-wave codes after the head
        filter, use another killed-wave code from that same zodiac even if it
        touches the killed head. This is exposed in metadata, never hidden.
    ...
    for z in cold3:
        # STRICT: only the two killed wave×parity groups can enter a cold zodiac.
        eligible=[
          n for n in ranked
          if zmap.get(n)==z
          and wave_parity_of(n) in killed_wp
        ]
        safe=[n for n in eligible if not killed_head or head_of(n)!=killed_head]
        ...
        # Still killed-wave only. If head filtering leaves fewer than 2,
        # the cold-zodiac "2 killed codes" rule has priority as requested.
        if len(picks)<2:
            for n in eligible:
                if n in picks:
                    continue
        ...
        # No non-killed code fallback. If the mapping genuinely cannot offer
        # 2 killed-wave codes, the shortage is kept visible instead of cheating.
        picks=picks[:2]
        cold_codes[z]=picks
        for n in picks:
    ...
    # Main pool: cold zodiacs cannot receive anything else.
    for n in ranked:
        if len(selected)>=27:
            break
        if n in selected:
    ...
    # Need more? Relax only wave×parity for NON-cold zodiacs.
    forced_fill=[]
    if len(selected)<27:
        for n in ranked:
            if len(selected)>=27:
    ...
    # Last resort for code count: keep cold-zodiac strictness untouched.
    if len(selected)<27:
        for n in ranked:
            if len(selected)>=27:
                break
    ...
    shortages={z:max(0,2-len(cold_codes.get(z,[]))) for z in cold3}
    shortages={z:v for z,v in shortages.items() if v>0}
    ...
    meta.update({
      ...
      "cold_shortages":shortages,
      "cold_head_override":head_override,
...
def _stats27_v5():
    profiles=[TEN27_V5_PROFILE,TEN27_V5_CORE_PROFILE,TEN27_V5_MOBILE_PROFILE,TEN27_V5_28_PROFILE]
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        c=connect()
        try:
    ...
    for x in rows:
        q=str(x["target_issue"])
        by_issue.setdefault(q,{})[str(x["profile"])]=dict(x)
    audit_map={str(x["target_issue"]):str(x["regime"] or "") for x in audits}
    issues=sorted([q for q,d in by_issue.items() if TEN27_V5_PROFILE in d],key=lambda x:int(x))

    groups={}
    order=[]
    for q in issues:
        tok=_long27_v5_parse(audit_map.get(q,""))
        rs=str(tok.get("round") or q)
        if rs not in groups:
            groups[rs]=[]
            order.append(rs)
        groups[rs].append(q)
    ...
    for rs in order:
        chunk=groups[rs]
        settled=[q for q in chunk if int(by_issue[q][TEN27_V5_PROFILE]["settled"] or 0)==1]
        hits=sum(int(by_issue[q][TEN27_V5_PROFILE].get("hit24",0) or 0) for q in settled)
        summaries.append({
          "start":rs,
          "end":_issue_add(rs,9),
          "n":len(settled),
          "hits":hits,
          "misses":max(0,len(settled)-hits)
        })

    empty={"start":0,"end":0,"n":0,"hits":0,"misses":0}
    current=summaries[-1] if summaries else dict(empty)
    complete=next((g for g in reversed(summaries) if g["n"]>=10),None)
    return {
      "current":current,
      "last_complete":complete,
      "rounds_completed":sum(1 for g in summaries if g["n"]>=10),
      "overall":_profile_hit_stats(TEN27_V5_PROFILE,60)
    }
...
def _stats27_v7():
    ...
    for rs in order:
        ...
        summaries.append({
          "start":rs,"end":_issue_add(rs,9),"n":len(settled),
          "hits":hits,"misses":max(0,len(settled)-hits)
        })
...
def _stats27_v6():
    ...
    return {
      ...
      "last_complete":next((g for g in reversed(summaries) if g["n"]>=10),None),
      "rounds_completed":sum(1 for g in summaries if g["n"]>=10),
      "overall":_profile_hit_stats(TEN27_V6_PROFILE,60)
    }

def _diagnostics27_v7(window=60):
    base=_strategy_diagnostics(TEN27_V7_PROFILE,window)
    with db_lock:
        c=connect()
        try:
            rows=c.execute("""SELECT target_issue,selected_codes,actual_special,actual_zodiac,
                                     selected_hit,killed_head,snapshot_json
                              FROM strategy_audit
                              WHERE profile=? AND settled=1
                              ORDER BY CAST(target_issue AS INTEGER) DESC
                              LIMIT ?""",(TEN27_V7_PROFILE,int(window))).fetchall()
        finally:
            c.close()
    ...
    for row in rows:
        d=dict(row)
        n=int(d.get("actual_special") or 0)
        if not n:
            continue
        snap=_audit_snapshot_load(d.get("snapshot_json",""))
        raw=set(int(x) for x in (snap.get("raw_top27") or []) if str(x).isdigit())
        selected=set(_csv_nums(d.get("selected_codes") or ""))
        z=normalize_z(str(d.get("actual_zodiac") or ""))
        cold3=set(snap.get("coldest3") or [])
        cold_codes=snap.get("cold3_codes") or {}
        killed_wp=set(snap.get("killed_wave_parity") or [])
        killed_head=str(snap.get("killed_head") or d.get("killed_head") or "")
        zcounts=snap.get("zodiac_counts") or {}

        if n in raw:
            raw_hits+=1
        if int(d.get("selected_hit") or 0)==1:
            reasons["命中"]+=1
            continue
        ...
        if n not in raw:
            reasons["底层模型没抓到"]+=1
        elif z in cold3 and n not in set(int(x) for x in (cold_codes.get(z) or [])):
            reasons["冷肖每肖2码没覆盖"]+=1
        elif killed_head and head_of(n)==killed_head:
            reasons["0/4头杀错"]+=1
        elif wave_parity_of(n) in killed_wp:
            reasons["波色单双杀错"]+=1
        elif int(zcounts.get(z,0) or 0)>=3:
            reasons["生肖3码上限挤出"]+=1
        else:
            reasons["结构截断"]+=1

    raw_n=len(rows)
    base["rule_failure_reasons"]=dict(reasons)
    base["raw_top27_hits"]=raw_hits
    base["raw_top27_n"]=raw_n
    base["raw_top27_rate"]=round(100*raw_hits/raw_n,1) if raw_n else 0.0
    base["final_misses"]=misses
    return base


def _long27_v7_choose_filter(r,profile,forced_head=None):
    """Choose the two weak wave×parity groups with cold-3 feasibility.

    Priority:
      1) weaker pair by previous-20 frequency;
      2) every cold zodiac must have >=2 codes from those killed groups;
      3) prefer solutions that also avoid the killed 0/4 head.

    If no pair can satisfy the head restriction, the pair must still satisfy
    the user's stricter rule: each cold zodiac has 2 codes from the killed
    wave×parity groups. Head collision is then recorded explicitly.
    """
    base=_long27_v4_window20(r)
    if forced_head in ("0头","4头",""):
        base["killed_head"]=forced_head

    counts=base.get("wave_counts") or {}
    killed_head=str(base.get("killed_head") or "")
    cold3=_long27_v4_cold3(r,profile)
    zmap=_number_zodiac_map(r)
    keys=list(WAVE_PARITY_KEYS)
        ...
        for j in range(i+1,len(keys)):
            ...
            for z in cold3:
                raw=[
                  ...
                  if zmap.get(n)==z and wave_parity_of(n) in pset
                ]
                safe=[
                  n for n in raw
                  if not killed_head or head_of(n)!=killed_head
                ]
                raw_counts[z]=len(raw)
                safe_counts[z]=len(safe)

            weak_sum=int(counts.get(pair[0],0))+int(counts.get(pair[1],0))
            weak_max=max(int(counts.get(pair[0],0)),int(counts.get(pair[1],0)))
            pairs.append({
              "pair":pair,
              "weak_sum":weak_sum,
              "weak_max":weak_max,
              ...
              "safe_ok":all(v>=2 for v in safe_counts.values()),
              "raw_ok":all(v>=2 for v in raw_counts.values()),
              "min_safe":min(safe_counts.values()) if safe_counts else 0,
              "min_raw":min(raw_counts.values()) if raw_counts else 0,
            })

    # Weakest feasible pair wins.
    safe=[x for x in pairs if x["safe_ok"]]
    if safe:
        chosen=min(safe,key=lambda x:(x["weak_sum"],x["weak_max"],x["pair"]))
        mode="冷三肖每肖2码+杀头均可满足"
    else:
        raw=[x for x in pairs if x["raw_ok"]]
        if raw:
            chosen=min(raw,key=lambda x:(x["weak_sum"],x["weak_max"],x["pair"]))
            mode="冷三肖每肖2码可满足；部分码需覆盖杀头"
        else:
            # Mathematical fallback: maximize cold-zodiac coverage, then keep
            # the weakest available pair. Never substitute non-killed groups.
            chosen=min(
              pairs,
              key=lambda x:(-x["min_raw"],x["weak_sum"],x["weak_max"],x["pair"])
            )
            mode="无完整可行双类；保留最接近的两弱类并暴露缺口"

    base["killed_wave_parity"]=list(chosen["pair"])
    base["coldest3"]=cold3
    base["pair_feasibility"]={
      "mode":mode,
      "safe_ok":bool(chosen["safe_ok"]),
      "raw_ok":bool(chosen["raw_ok"]),
      "safe_counts":chosen["safe_counts"],
      "raw_counts":chosen["raw_counts"],
      "weak_sum":chosen["weak_sum"],
      "all_pairs_checked":len(pairs)
    }
    return base


def _diagnostics27_v6(window=60):
    base=_strategy_diagnostics(TEN27_V6_PROFILE,window)
    with db_lock:
        c=connect()
        try:
            rows=c.execute("""SELECT target_issue,selected_codes,actual_special,actual_zodiac,
                              ...
                              WHERE profile=? AND settled=1
                              ORDER BY CAST(target_issue AS INTEGER) DESC
                              LIMIT ?""",(TEN27_V6_PROFILE,int(window))).fetchall()
        finally:
            c.close()
...
def _long27_v4_window20(r):
    """Clarified user rule for one 10-period long-code round.

    Look back exactly 20 specials:
    1) rank 红单/红双/蓝单/蓝双/绿单/绿双 and kill the two least frequent;
    2) compare 0头 vs 4头 and kill the less frequent one; tie = no head kill.
    """
    block=r[:20]
    wp=Counter()
    heads=Counter()
    ...
    c0=int(heads.get("0头",0))
    c4=int(heads.get("4头",0))
    killed_head=""
    if c0<c4:
        killed_head="0头"
    elif c4<c0:
    ...
    return {
      ...
      "killed_wave_parity":list(killed_wp),
      "head_counts":{"0头":c0,"4头":c4},
      "killed_head":killed_head
    }

def _long27_v4_token(filt,segment=1,switches=0):
    waves=",".join(filt.get("killed_wave_parity") or [])
    head=str(filt.get("killed_head") or "")
    return f"LONG27V4|waves={waves}|head={head}|segment={int(segment)}|switches={int(switches)}"

def _long27_v4_parse(token):
    out={"waves":[],"head":"","segment":1,"switches":0}
    t=str(token or "")
    if not t.startswith("LONG27V4|"):
        return out
    for part in t.split("|")[1:]:
        if "=" not in part:
            continue
...
def _long27_v4_round_context(target_issue):
    """Sequential honest groups of 10 using V4 profiles only."""
    with db_lock:
        c=connect()
        try:
            full=c.execute("""SELECT target_issue,special24,settled
                              ...
                              WHERE profile=?
                              ORDER BY CAST(target_issue AS INTEGER) ASC""",
                           (TEN27_V4_PROFILE,)).fetchall()
            cores=c.execute("""SELECT target_issue,special24
                               FROM prediction_log
                               WHERE profile=?
                               ORDER BY CAST(target_issue AS INTEGER) ASC""",
                            (TEN27_V4_CORE_PROFILE,)).fetchall()
            mobiles=c.execute("""SELECT target_issue,special24
                                 FROM prediction_log
                                 WHERE profile=?
                                 ORDER BY CAST(target_issue AS INTEGER) ASC""",
                              (TEN27_V4_MOBILE_PROFILE,)).fetchall()
            audits=c.execute("""SELECT target_issue,regime
                                FROM strategy_audit
                                WHERE profile=?
                                ORDER BY CAST(target_issue AS INTEGER) ASC""",
    ...
    return {
      ...
      "prev_filter":_long27_v4_parse(prev_token)
    }

def _long27_v4_head_streak(r,killed_head,max_check=3):
    """Only the killed 0/4 head can trigger a trend switch."""
    if not killed_head:
        return 0
    run=0
...
def _long27_v4_cold3(r,profile):
    zheat=_hot_zodiac_scores(r,profile)
...
def _long27_v4_rescue28(r,profile,base27,filt):
    ...
    zmap=_number_zodiac_map(r)
    occupied=set(base27)
    zcount=Counter(zmap.get(n) for n in base27 if zmap.get(n))
    killed_head=str(filt.get("killed_head") or "")
    ranked=sorted(range(1,50),key=lambda n:(-score.get(n,-1e9),n))
    ...
    return {
      ...
      "ensemble_score":round(sc,3),
      "message":f"长码27外强覆盖码 {n:02d}，本期临时补为第28码"
    }

def _stats27_v4():
    profiles=[TEN27_V4_PROFILE,TEN27_V4_CORE_PROFILE,TEN27_V4_MOBILE_PROFILE,TEN27_V4_28_PROFILE]
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        c=connect()
        try:
    ...
    issues=sorted([i for i,d in by_issue.items() if TEN27_V4_PROFILE in d],key=lambda x:int(x))
    groups=[]
    for i in range(0,len(issues),10):
        chunk=issues[i:i+10]
        if not chunk:
            continue
        settled=[q for q in chunk if int(by_issue[q][TEN27_V4_PROFILE]["settled"] or 0)==1]
        core_hits=mobile_rescue=rescue28=final_hits=0
        segments=[]
        for q in chunk:
            tok=_long27_v4_parse(audit_map.get(q,""))
            segments.append(int(tok.get("segment",1)))
        for q in settled:
            d=by_issue[q]
            ch=int((d.get(TEN27_V4_CORE_PROFILE) or {}).get("hit24",0) or 0)
            mh=int((d.get(TEN27_V4_MOBILE_PROFILE) or {}).get("hit24",0) or 0)
            rh=int((d.get(TEN27_V4_28_PROFILE) or {}).get("hit24",0) or 0)
            fh=int((d.get(TEN27_V4_PROFILE) or {}).get("hit24",0) or 0)
            core_hits+=ch
            if mh and not ch:
                mobile_rescue+=1
            if rh and not ch and not mh:
    ...
    return {
      ...
      "rounds_completed":sum(1 for g in groups if g["n"]>=10),
      "overall":_profile_hit_stats(TEN27_V4_PROFILE,60)
    }
...
def _predict27_dynamic_b(r, profile):
    ...
    只使用最新10期：
      ...
      - 冷3肖各2码、其他肖最多3码
    新增：
      - 0~9尾最近10期 + 最近4期强弱，权重10%
      - 尾数只影响排名，不硬杀任何一尾
    """
    nums=list(range(1,50))
    recent=list(r[:10])

    # Zodiac mapping inferred ONLY from these 10 draws.
    ...
    # --------------------------------------------------------
    # Original statistics, now strictly latest10 + tail number.
    # --------------------------------------------------------
    freq=Counter()
    wave_w=Counter()
    parity_w=Counter()
    ...
    tail20=Counter()
    totalw=0.0
    gap={n:10 for n in nums}
    zgap={z:10 for z in ALL_ZODIACS}
    zraw=Counter()
    ...
    parity_share={p:(parity_w[p]/totalw if totalw else .5) for p in ["单","双"]}
    zodiac_share={z:(zodiac_w[z]/totalw if totalw else 1/12) for z in ALL_ZODIACS}

    # Short acceleration remains inside the latest10 only.
    w6=Counter(); p6=Counter(); z6=Counter(); t6=Counter()
    ...
    st20=sum(tail20.values()) or 1.0
    st6=sum(t6.values()) or 1.0

    # v63: tails that did NOT appear in the latest 10 special numbers.
    seen_tails=set()
    for x in recent:
        try:
            seen_tails.add(int(x["special"])%10)
        except Exception:
            pass
    missing_tails=[t for t in range(10) if t not in seen_tails]

    # Original B score first, untouched.
    base_score={}
    for n in nums:
        z=zmap.get(n)
    ...
    # Tail = 10% soft score. No tail can ever be hard-excluded.
    tail_raw={
      t:.72*(tail20.get(t,0.0)/st20)+.28*(t6.get(t,0.0)/st6)
      for t in range(10)
    }
    tail_strength=_norm_values(tail_raw,range(10))
    score={
      n:.90*base_score.get(n,.5)+.10*tail_strength.get(n%10,.5)
      for n in nums
    }
    score=_norm_values(score,nums)

    # --------------------------------------------------------
    # Original dynamic head kill, unchanged.
    # --------------------------------------------------------
    ...
    h20=head_strength(recent,3.6)
    h8=head_strength(recent[:4],1.7)
    ...
    # Original cold3 logic, unchanged.
    cold3=sorted(ALL_ZODIACS,key=lambda z:(zraw.get(z,0),-zgap.get(z,20),z))[:3]
    coldset=set(cold3)
    ...
    for z in cold3:
        safe=[n for n in ranked_all if zmap.get(n)==z and head_of(n)!=killed_head]
        picks=safe[:2]
        if len(picks)<2:
            for n in ranked_all:
    ...
    # --------------------------------------------------------
    # v63 missing-tail rescue:
    # If a tail did not appear in the latest 10 specials, guarantee ONE code
    # of that tail in B.  The rescue candidate must avoid the killed head.
    # It REPLACES a weak code, so B remains exactly 27 codes.
    # Cold3=2 each and other-zodiac<=3 are preserved.
    # --------------------------------------------------------
    missing_tail_rescue={}
    tail_protected=set()

    def _pick_drop_for_tail(add_n):
        add_z=zmap.get(add_n)
        # If adding would exceed a zodiac cap, swap within the same zodiac.
        if add_z in coldset and zcount.get(add_z,0)>=2:
            same=[n for n in selected
                  if zmap.get(n)==add_z
                  and n not in tail_protected
                  and n%10 not in missing_tails]
            if same:
                return min(same,key=lambda n:(score.get(n,0),n))
        if add_z not in coldset and add_z and zcount.get(add_z,0)>=3:
            same=[n for n in selected
                  if zmap.get(n)==add_z
                  and n not in tail_protected
                  and n%10 not in missing_tails]
        ...
        # Otherwise drop the weakest non-cold, non-protected code.
        pool=[n for n in selected
              if zmap.get(n) not in coldset
              and n not in tail_protected
              and n%10 not in missing_tails]
        if not pool:
            pool=[n for n in selected
                  if n not in tail_protected
                  and n%10 not in missing_tails]
        return min(pool,key=lambda n:(score.get(n,0),n)) if pool else None

    for t in missing_tails:
        # If the normal 27-code selection already contains this absent tail,
        # record it as naturally covered and do not force another one.
        existing=[n for n in selected if n%10==t and head_of(n)!=killed_head]
        if existing:
            keep=max(existing,key=lambda n:(score.get(n,0),-n))
            tail_protected.add(keep)
            missing_tail_rescue[str(t)]={
              "code":keep,"action":"natural","killed_head":killed_head
            }
            continue

        candidates=[n for n in ranked_all
                    if n not in selected
                    and n%10==t
                    and head_of(n)!=killed_head]
        if not candidates:
            continue

        # Prefer candidates that can preserve the existing zodiac rules.
        feasible=[]
        for n in candidates:
            z=zmap.get(n)
            if not z:
                continue
            if z in coldset:
                feasible.append(n)  # can swap within the same cold zodiac
            elif zcount.get(z,0)<=3:
                feasible.append(n)
        if feasible:
            candidates=feasible

        add_n=max(candidates,key=lambda n:(score.get(n,0),-n))
        drop_n=_pick_drop_for_tail(add_n)
        if drop_n is None:
            continue

        dz=zmap.get(drop_n)
        az=zmap.get(add_n)
        selected[selected.index(drop_n)]=add_n
        if dz:
            zcount[dz]=max(0,zcount.get(dz,0)-1)
        if az:
            zcount[az]+=1
        tail_protected.add(add_n)
        missing_tail_rescue[str(t)]={
          "code":add_n,"replaced":drop_n,"action":"forced",
          "killed_head":killed_head
        }

    selected=selected[:27]
    counts=dict(Counter(zmap.get(n) for n in selected if zmap.get(n)))
    top_tails=sorted(range(10),key=lambda t:(-tail_strength.get(t,0),t))[:3]

    return selected,{
      "mode":"B组原算法+尾数软评分·纯近10期",
      "window":len(recent),
      "window_issues":[str(x["issue"]) for x in recent if x["issue"]],
      "latest_issue":str(recent[0]["issue"]) if recent else "",
      "recalc_each_issue":True,
      "independent":True,
      "uses_only_recent10":True,
      "killed_head":killed_head,
      ...
      "zodiac_map":{str(n):zmap.get(n,"") for n in nums},
      "cold_each":2,
      "other_zodiac_cap":3,
      "head_override":head_override,
      "ranked49":ranked_all,
      "score49":{str(n):round(float(score.get(n,0)),6) for n in nums},
      "tail_weight_pct":10.0,
      "tail_soft_only":True,
      "tail_top3":top_tails,
      "tail_strength":{str(t):round(float(tail_strength.get(t,0)),4) for t in range(10)},
      "missing_tails":missing_tails,
      "missing_tail_rescue":missing_tail_rescue,
      "missing_tail_rule":"近10期未出尾，每尾至少1码；按杀码避开被杀头",
      "filter20":{
        ...
        "tail_counts":{str(t):round(tail20[t],3) for t in range(10)}
      },
      "regime":f"B近10期·缺尾补码·尾数10%软评·杀{killed_head}·冷3肖各2",
      "audit_regime":f"B27V63|window=10|missingtail={','.join(str(x) for x in missing_tails)}|kill={killed_head}|cold={','.join(cold3)}"
    }


def _predict22_dynamic_c(r, profile):
    """C组22码 v62：近10期独立22位优化。

    Uses the same latest10 evidence as B (including tail soft score),
    but DOES NOT truncate B's selected 27 codes.

    C-specific structure:
      - rebuild directly from raw 49-number ranking;
      - killed head is a soft penalty, never hard delete;
      - zodiac "3-code cap" is a soft crowding penalty, never hard delete;
      - coldest3 reserve a TOTAL of 4~6 seats, not rigid 2 each.
    """
    _bcodes,bmeta=_predict27_dynamic_b(r,profile)

    ranked=list(bmeta.get("ranked49") or [])
    if not ranked:
        ranked=list(range(1,50))

    score49={int(k):float(v) for k,v in (bmeta.get("score49") or {}).items()}
    if not score49:
        score49={n:(49-i)/49.0 for i,n in enumerate(ranked)}

    zmap={int(k):v for k,v in (bmeta.get("zodiac_map") or {}).items()}
    cold3=list(bmeta.get("coldest3") or [])[:3]
    coldset=set(cold3)
    killed_head=str(bmeta.get("killed_head") or "")
    hconf=str(bmeta.get("head_confidence") or "弱")

    # Head is SOFT for C.
    head_penalty={"强":.060,"中":.040,"弱":.022}.get(hconf,.025)

    # First independent score: raw B evidence + only soft structural penalties.
    base_adjusted={}
    for n in ranked:
        sc=float(score49.get(n,0.0))
        if killed_head and head_of(n)==killed_head:
            sc-=head_penalty
        # tiny diversity encouragement for cold3, but not "2 each".
        if zmap.get(n) in coldset:
            sc+=.008
        base_adjusted[n]=sc

    # Dynamic cold3 total: if raw top22 already contains 4~6 cold-zodiac codes,
    # keep that natural count; otherwise bring it into the 4~6 safety band.
    raw_cold22=sum(1 for n in ranked[:22] if zmap.get(n) in coldset)
    cold_target=max(4,min(6,raw_cold22))

    # Greedy 22-seat selection with a SOFT zodiac crowding penalty.
    selected=[]
    zcount=Counter()
    remaining=set(ranked)
    while remaining and len(selected)<22:
        best=None
        bestv=-1e9
        for n in remaining:
            z=zmap.get(n)
            crowd=max(0,zcount.get(z,0)-2) if z else 0
            # 4th code of one zodiac is discouraged, not forbidden.
            v=base_adjusted.get(n,0.0)-.035*crowd
            if v>bestv or (abs(v-bestv)<1e-12 and (best is None or n<best)):
                bestv=v; best=n
        if best is None:
            break
    ...
    # Cold3 total reserve 4~6, not 2-per-zodiac.
    def cold_count(arr):
        return sum(1 for n in arr if zmap.get(n) in coldset)

    while cold_count(selected)<cold_target:
        cold_unselected=[n for n in ranked if n not in selected and zmap.get(n) in coldset]
        noncold_selected=[n for n in selected if zmap.get(n) not in coldset]
        if not cold_unselected or not noncold_selected:
            break
        add=max(cold_unselected,key=lambda n:(base_adjusted.get(n,0),-n))
        drop=min(noncold_selected,key=lambda n:(base_adjusted.get(n,0),n))
        selected[selected.index(drop)]=add

    while cold_count(selected)>6:
        cold_selected=[n for n in selected if zmap.get(n) in coldset]
        noncold_unselected=[n for n in ranked if n not in selected and zmap.get(n) not in coldset]
        if not cold_selected or not noncold_unselected:
            break
        drop=min(cold_selected,key=lambda n:(base_adjusted.get(n,0),n))
        add=max(noncold_unselected,key=lambda n:(base_adjusted.get(n,0),-n))
        selected[selected.index(drop)]=add

    # v63: same missing-tail concept as B.
    # C keeps its 22-seat size.  Rescue candidate avoids the killed head first;
    # head remains a soft rule for ordinary C ranking, but missing-tail rescue
    # explicitly uses the killed-head decision as requested.
    c_missing_tails=list(bmeta.get("missing_tails") or [])
    c_tail_rescue={}
    c_tail_protected=set()

    for t in c_missing_tails:
        existing=[n for n in selected if n%10==int(t) and head_of(n)!=killed_head]
        if existing:
            keep=max(existing,key=lambda n:(base_adjusted.get(n,0),-n))
            c_tail_protected.add(keep)
            c_tail_rescue[str(t)]={"code":keep,"action":"natural","killed_head":killed_head}
            continue

        cand=[n for n in ranked
              if n not in selected
              and n%10==int(t)
              and head_of(n)!=killed_head]
        if not cand:
        ...
        # Do not push cold3 above 6 if avoidable.
        safe=[n for n in cand
              if zmap.get(n) not in coldset or cold_count(selected)<6]
        if safe:
            cand=safe
        add=max(cand,key=lambda n:(base_adjusted.get(n,0),-n))

        # Protect already-rescued tails and preserve the cold3 minimum.
        drops=[]
        for n in selected:
            if n in c_tail_protected:
                continue
            if n%10 in c_missing_tails:
                continue
            if zmap.get(n) in coldset and cold_count(selected)<=cold_target:
                continue
            drops.append(n)
        if not drops:
            drops=[n for n in selected if n not in c_tail_protected]
        if not drops:
            continue

        drop=min(drops,key=lambda n:(base_adjusted.get(n,0),n))
        selected[selected.index(drop)]=add
        c_tail_protected.add(add)
        c_tail_rescue[str(t)]={
          "code":add,"replaced":drop,"action":"forced",
          "killed_head":killed_head
        }

    # Final order follows independent adjusted strength; display remains sorted later.
    selected=sorted(set(selected),key=lambda n:(-base_adjusted.get(n,-1e9),n))[:22]
    counts=dict(Counter(zmap.get(n) for n in selected if zmap.get(n)))
    head_selected=sum(1 for n in selected if killed_head and head_of(n)==killed_head)

    meta=dict(bmeta)
    meta.update({
      "mode":"C组独立22位优化·纯近10期",
      "code_count":22,
      "source":"同一近10期证据·独立22位优化",
      "independent22":True,
      "ranked49":sorted(ranked,key=lambda n:(-base_adjusted.get(n,-1e9),n)),
      "score22_base":{str(n):round(base_adjusted.get(n,0.0),6) for n in ranked},
      "cold_target_total":cold_target,
      "cold_actual_total":cold_count(selected),
      "cold_each":None,
      "other_zodiac_cap":None,
      "zodiac_counts":counts,
      "head_soft_penalty":round(head_penalty,3),
      "head_selected_count":head_selected,
      "head_is_soft":True,
      "zodiac_cap_is_soft":True,
      "tail_weight_pct":10.0,
      "missing_tails":c_missing_tails,
      "missing_tail_rescue":c_tail_rescue,
      "missing_tail_rule":"近10期未出尾，每尾至少1码；补码优先避开杀头",
      "audit_regime":f"C22V63|window=10|missingtail={','.join(str(x) for x in c_missing_tails)}|headsoft={killed_head}|coldtotal={cold_count(selected)}"
    })
...
def _stats27b_v63(target_issue=None):
    """B v62 algorithm uses latest10; performance card still groups 20 results."""
    with db_lock:
        c=connect()
        try:
    ...
    if target and target in issues:
        idx=issues.index(target); gstart=(idx//20)*20
        chunk=items[gstart:gstart+20]
        start_issue=str(chunk[0]["target_issue"]) if chunk else target
    else:
        rem=total%20
        if rem==0:
            ...
            chunk=items[total-rem:]
            start_issue=str(chunk[0]["target_issue"])

    settled=[x for x in chunk if int(x.get("settled") or 0)==1]
    n=len(settled); hits=sum(int(x.get("hit24") or 0) for x in settled)
    hk_n=len(audits); hk_ok=sum(int(x["head_kill_success"] or 0) for x in audits)
    baseline=(sum(1.0-float(HEAD_BASE.get(str(x["killed_head"]),10/49))
                  for x in audits)/hk_n) if hk_n else 0.0

    return {
      "start":start_issue,"end":_issue_add(start_issue,19) if start_issue else "",
      "n":n,"hits":hits,"misses":max(0,n-hits),
      "rate":round(100*hits/n,1) if n else 0.0,
      "round_size":20,"round_complete":bool(n>=20),"round_no":(total//20+1),
      "version_lifetime":_profile_lifetime_stats(B27_V63_PROFILE),
      "lifetime":_profiles_lifetime_stats([B27_V52_PROFILE,B27_V58_PROFILE,B27_V63_PROFILE]),
      "head_kill_n":hk_n,"head_kill_success":hk_ok,
      "head_kill_rate":round(100*hk_ok/hk_n,1) if hk_n else 0.0,
      "head_kill_baseline":round(100*baseline,1) if hk_n else 0.0,
      "algorithm_window":10
    }

def _stats27b_v58(target_issue=None):
    """B v58: 20-period round on new algorithm + cumulative old+new B history."""
    with db_lock:
        c=connect()
        try:
            all_rows=c.execute("""SELECT target_issue,hit24,settled
                                  ...
                                  WHERE profile=?
                                  ORDER BY CAST(target_issue AS INTEGER) ASC""",
                               (B27_V58_PROFILE,)).fetchall()
            audits=c.execute("""SELECT killed_head,head_kill_success
                                FROM strategy_audit
                                WHERE profile=? AND settled=1 AND killed_head<>''
                                ORDER BY CAST(target_issue AS INTEGER) DESC
                                LIMIT 60""",(B27_V58_PROFILE,)).fetchall()
        finally:
            c.close()
    ...
    return {
      ...
      "round_complete":bool(n>=20),
      "round_no":(total//20+1),
      "version_lifetime":_profile_lifetime_stats(B27_V58_PROFILE),
      "lifetime":_profiles_lifetime_stats([B27_V52_PROFILE,B27_V58_PROFILE]),
      "head_kill_n":hk_n,
    ...
    with db_lock:
        ...
        try:
            rows=c.execute("""SELECT hit24 FROM prediction_log
                              WHERE profile=? AND settled=1
                              ORDER BY CAST(target_issue AS INTEGER) DESC
                              LIMIT ?""",(B27_V50_PROFILE,int(window))).fetchall()
        finally:
            c.close()
    n=len(rows)
    hits=sum(int(x["hit24"] or 0) for x in rows)
...
def _pool_5head_kill_decision(r,profile,strategy="27"):
    """Six-strategy pool chooses the weakest ONE of 0/1/2/3/4 heads.

    Each formal model (趋势/生肖/冷热/波色/单双/纠错) scores every head by
    the average score of numbers in that head.  Real model-performance weights
    are then fused.  The head with the largest weakness score is killed.
    """
    heads=["0头","1头","2头","3头","4头"]
    models=_specialist_model_scores(r,profile,strategy)
    perf=_pool_performance()
    weights=perf.get("weights") or {k:1/max(1,len(models)) for k in models}

    model_head={}
    weakest_votes=Counter()
    weighted_strength={h:0.0 for h in heads}

    for k,sm in models.items():
        raw={}
        for h in heads:
            pool=[n for n in range(1,50) if head_of(n)==h]
            raw[h]=sum(float(sm.get(n,.5)) for n in pool)/max(1,len(pool))
        norm=_norm_values(raw,heads)
        model_head[k]={h:round(float(norm[h]),4) for h in heads}
        weak=min(heads,key=lambda h:(norm[h],h))
        weakest_votes[weak]+=float(weights.get(k,0))
        for h in heads:
            weighted_strength[h]+=float(weights.get(k,0))*float(norm[h])

    # Normalize aggregate strength.  Lower strength = weaker.
    weighted_strength=_norm_values(weighted_strength,heads)
    vote_max=max(weakest_votes.values()) if weakest_votes else 1.0
    weakness={}
    for h in heads:
        vote_component=(float(weakest_votes.get(h,0))/vote_max) if vote_max else 0.0
        weakness[h]=.82*(1.0-float(weighted_strength[h]))+.18*vote_component

    ordered=sorted(heads,key=lambda h:(-weakness[h],h))
    killed=ordered[0]
    gap=float(weakness[ordered[0]]-weakness[ordered[1]]) if len(ordered)>1 else 0.0
    conf="强" if gap>=.12 else ("中" if gap>=.055 else "弱")

    return {
      "killed_head":killed,
      "confidence":conf,
      "gap":round(gap,4),
      "weakness":{h:round(float(weakness[h]),4) for h in heads},
      "strength":{h:round(float(weighted_strength[h]),4) for h in heads},
      "votes":{h:round(float(weakest_votes.get(h,0))*100,1) for h in heads},
      "model_head":model_head,
      "model_weights":{k:round(float(weights.get(k,0))*100,1) for k in models}
    }

def _a27_v62_context(target_issue):
    """Current A-v62 fixed 10-period round, including its fixed killed head."""
    with db_lock:
        c=connect()
        try:
            rows=c.execute("""SELECT target_issue,special24,settled
    ...
    def build(chunk,pos):
        ...
        if start:
            with db_lock:
                c=connect()
                try:
                    a=c.execute("""SELECT killed_head FROM strategy_audit
                                   WHERE target_issue=? AND profile=?""",
                                (start,A27_V62_PROFILE)).fetchone()
                finally:
                    c.close()
            killed=str(a["killed_head"] or "") if a else ""
        return {
          "new_round":False,"start":start,"end":_issue_add(start,9),
          "position":pos,"codes":codes,"killed_head":killed
        }
    ...
    if target and target in issues:
        ...
        return build(chunk,idx-gstart+1)

    rem=len(items)%10
    if rem:
        chunk=items[len(items)-rem:]
        return build(chunk,rem+1)

    return {
      "new_round":True,"start":target,"end":_issue_add(target,9) if target else "",
      "position":1,"codes":[],"killed_head":""
    }

def _predict27_tenblock_v62(r,profile,target_issue):
    """A组 v62：10期固定长码 + 多策略池杀0~4其中一头。"""
    rc=_a27_v62_context(target_issue)

    # Existing round: codes and killed head are FIXED until the 10th issue ends.
    if not rc.get("new_round") and rc.get("codes"):
        ...
        killed=str(rc.get("killed_head") or "")
        return selected,{
          "block_start":rc["start"],"block_end":rc["end"],
          "round_position":rc["position"],
          "round_mode":"A多策略池杀一头·10期固定",
          "code_count":len(selected),
          "core23":selected[:23],"mobile4":selected[23:27],
          "ranked49":ranked49,
          "raw_top27":ranked49[:27],
          "killed_wave_parity":[],
          "killed_head":killed,
          "head_decision":{"killed_head":killed,"fixed_round":True},
          "coldest3":[],"cold3_codes":{},"cold_defense6":[],
          "zodiac_counts":{},"trend_switches":0,
          "rescue28":{"active":False,"code":None,"message":""},
          "simplified":True,
          "regime":f"A十期固定·多策略池杀{killed or '--'}",
          "audit_regime":f"A27V62|round={rc['start']}|kill={killed}"
        }

    # New round: choose weakest head ONCE, then build 27 codes excluding it.
    ...
    selected=[n for n in ranked49 if head_of(n)!=killed][:27]
    ...
    return selected,{
      ...
      "regime":f"A十期固定·六模型池杀{killed}",
      "audit_regime":f"A27V62|round={target_issue}|kill={killed}|conf={hd.get('confidence','')}"
    }
...
def _stats27_a_v62():
    ...
    return {
      ...
      "rounds_completed":sum(1 for g in groups if g["n"]>=10),
      "overall":_profile_hit_stats(A27_V62_PROFILE,60),
      "lifetime":_profiles_lifetime_stats([A27_V57_PROFILE,A27_V62_PROFILE]),
      "version_lifetime":_profile_lifetime_stats(A27_V62_PROFILE)
    }
...
def _predict27_tenblock(r, profile, target_issue):
    ...
    if rc.get("new_round") or not rc.get("core23"):
        active_filter=_long27_v7_choose_filter(r,profile)
        chosen27,_score,_spec,perf,meta=_build_long27_v5_codes(r,profile,active_filter)
        round_start=str(target_issue)
        position=1
        segment=1
        switches=0
        switched=False
        head_streak=0
        raw_top27=list(meta.get("ranked49") or [])[:27]
        pair_feasibility=active_filter.get("pair_feasibility") or {}
    else:
        prev=rc.get("prev_filter") or {}
        snap=rc.get("prev_snapshot") or {}
        prev_waves=list(snap.get("killed_wave_parity") or prev.get("waves") or [])
        prev_head=str(snap.get("killed_head") or prev.get("head") or "")
        round_start=str(snap.get("round_start") or rc.get("round_start") or target_issue)
        position=int(rc.get("position") or 1)
        segment=int(snap.get("trend_segment") or prev.get("segment") or 1)
        switches=int(snap.get("trend_switches") or prev.get("switches") or 0)

        chosen27=list(rc.get("core23") or [])[:23]+list(rc.get("mobile4") or [])[:4]
        active_filter={
          "window":20,
          "wave_counts":snap.get("wave_counts") or {},
          "head_counts":snap.get("head_counts") or {},
          "killed_wave_parity":prev_waves,
          "killed_head":prev_head,
          "pair_feasibility":snap.get("pair_feasibility") or {}
        }
        switched=False
        head_streak=_long27_v4_head_streak(r,prev_head,3)

        # Keep the exact round snapshot while the long-code set is unchanged.
        meta={
          "analog_samples":0,
          "ranked49":list(snap.get("ranked49") or []),
          "coldest3":list(snap.get("coldest3") or []),
          "cold3_codes":snap.get("cold3_codes") or {},
          "cold_defense6":list(snap.get("cold_defense6") or []),
          "cold_shortages":snap.get("cold_shortages") or {},
          "cold_head_override":list(snap.get("cold_head_override") or []),
          "forced_fill":[],
          "zodiac_counts":snap.get("zodiac_counts") or {}
        }
        raw_top27=list(snap.get("raw_top27") or (meta["ranked49"][:27] if meta["ranked49"] else []))
        pair_feasibility=snap.get("pair_feasibility") or {}

        # Killed head appears twice -> switch head, recheck latest 20,
        # and re-select the weakest TWO wave×parity groups that can still give
        # every cold zodiac two killed-group codes.
        if prev_head and head_streak>=2:
            opposite="4头" if prev_head=="0头" else "0头"
            active_filter=_long27_v7_choose_filter(r,profile,forced_head=opposite)
            chosen27,_score,_spec,perf,meta=_build_long27_v5_codes(r,profile,active_filter)
            raw_top27=list(meta.get("ranked49") or [])[:27]
            pair_feasibility=active_filter.get("pair_feasibility") or {}
            round_start=str(target_issue)
    ...
    zodiac_counts=dict(Counter(zmap.get(n) for n in chosen27 if zmap.get(n)))
    meta["zodiac_counts"]=zodiac_counts

    rescue28=_long27_v4_rescue28(r,profile,chosen27,active_filter)
    ...
    audit_regime=_long27_v5_token(active_filter,round_start,segment,switches)

    return selected,{
      "block_start":round_start,
      "block_end":_issue_add(round_start,9),
      "round_position":position,
      "round_mode":"10期长码·冷三肖可行性·原始Top27对照",
      "core23":core23,
      "mobile4":mobile4,
      "code_count":len(selected),
      ...
      "cold_shortages":meta.get("cold_shortages",{}),
      "cold_head_override":meta.get("cold_head_override",[]),
      "pair_feasibility":pair_feasibility,
      "forced_fill":meta.get("forced_fill",[]),
      "zodiac_counts":zodiac_counts,
      "raw_top27":raw_top27,
      "ranked49":meta.get("ranked49") or raw_top27,
      "rescue28":rescue28,
      "ten_horizon":{
...
def _record_strategy_audit(target_issue,profile,selected,meta):
    ...
    snapshot={
      "round_start":str(meta.get("block_start") or ""),
      "round_end":str(meta.get("block_end") or ""),
      "killed_wave_parity":list(meta.get("killed_wave_parity") or []),
      "killed_head":killed,
      "coldest3":list(meta.get("coldest3") or []),
      "cold3_codes":meta.get("cold3_codes") or {},
      "cold_defense6":list(meta.get("cold_defense6") or []),
      "raw_top27":list(meta.get("raw_top27") or (ranked[:27] if ranked else [])),
      "ranked49":list(ranked),
      "zodiac_counts":meta.get("zodiac_counts") or {},
      "trend_segment":int(meta.get("trend_segment") or 1),
      "trend_switches":int(meta.get("trend_switches") or 0),
      "pair_feasibility":meta.get("pair_feasibility") or {},
      "wave_counts":(meta.get("filter20") or {}).get("wave_counts") or {},
      "head_counts":(meta.get("filter20") or {}).get("head_counts") or {},
      "cold_shortages":meta.get("cold_shortages") or {},
      "cold_head_override":list(meta.get("cold_head_override") or [])
    }
    ...
    with db_lock:
        ...
        try:
            c.execute("""INSERT OR IGNORE INTO strategy_audit
              (target_issue,profile,ranked49,selected_codes,excluded_zodiacs,killed_head,regime,snapshot_json)
              VALUES (?,?,?,?,?,?,?,?)""",
    ...
    with db_lock:
        ...
        try:
            ...
            for row in rows:
                ...
                if str(row["profile"])==TEN27_V7_PROFILE:
                    snap=_audit_snapshot_load(row["snapshot_json"] if "snapshot_json" in row.keys() else "")
                    raw=set(int(x) for x in (snap.get("raw_top27") or []) if str(x).isdigit())
                    cold3=set(snap.get("coldest3") or [])
                    cold_codes=snap.get("cold3_codes") or {}
                    killed_wp=set(snap.get("killed_wave_parity") or [])
                    zcounts=snap.get("zodiac_counts") or {}
                    z=normalize_z(str(actual_zodiac or ""))
                    if hit:
                        reason="命中"
                    elif int(actual_special) not in raw:
                        reason="底层模型没抓到"
                    elif z in cold3 and int(actual_special) not in set(int(x) for x in (cold_codes.get(z) or [])):
                        reason="冷肖每肖2码没覆盖"
                    elif killed and head_success==0:
                        reason="0/4头杀错"
                    elif wave_parity_of(int(actual_special)) in killed_wp:
                        reason="波色单双杀错"
                    elif int(zcounts.get(z,0) or 0)>=3:
                        reason="生肖3码上限挤出"
                    else:
                        reason="结构截断"
                    cold_err=int(reason=="冷肖每肖2码没覆盖")
                else:
                    if hit:
                        reason="命中"
                    elif cold_err:
...
def _strategy_diagnostics(profile,window=60):
    ...
    kill_by_head={h:{"n":0,"hits":0} for h in ["0头","1头","2头","3头","4头"]}
    for x in rows:
...
def _profiles_lifetime_stats(profiles):
    ...
    if not profiles:
        return {"n":0,"hits":0,"misses":0,"rate":0.0}
    placeholders=",".join("?" for _ in profiles)
    with db_lock:
        c=connect()
        try:
            row=c.execute(f"""SELECT COUNT(*) AS n,
                                      COALESCE(SUM(CASE WHEN hit24=1 THEN 1 ELSE 0 END),0) AS hits
                               FROM prediction_log
                               WHERE profile IN ({placeholders}) AND settled=1""",
                          tuple(profiles)).fetchone()
        finally:
            c.close()
